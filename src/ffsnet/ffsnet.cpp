 ...
 */
int ffs_mkdir(const char *proto, const char *remote_ip, const char *server_port,
	...
	if (0 != getaddrinfo(remote_ip, server_port, &hints, &peer)) {
		cout << "incorrect server/peer address. " << remote_ip << ":"
				<< server_port << endl;
		return -1;
	}
 ...
 */
int ffs_rmfile(const char *proto, const char *remote_ip,
 ...
 */
static int _recvfile_udt(const char *remote_ip, const char *server_port,
 ...
 */
static int _sendfile_udt(const char *remote_ip, const char *server_port,
 ...
 */
int ffs_recvfile(const char *proto, const char *remote_ip,
		const char *server_port, const char *remote_filename,
	...
	if (-1
			== _recvfile_udt(remote_ip, server_port, remote_filename,
					local_filename)) {
 ...
 * wrapper for upload
 */
int ffs_sendfile(const char *proto, const char *remote_ip,
	...
	if (-1
			== _sendfile_udt(remote_ip, server_port, local_filename,
					remote_filename)) {
