...
#include "replication/NeighborUtil-c.h"
...
//	printf("\n =====DFZ debug: %s \n", "zht_init() succeeded. ");
	...
	c_setNeighborSeeds("./src/zht/neighbor"); //xbz: init shortcut to access neighbors
...
ENTRY* ht_search(const char *key) {
	...
	if (NULL != ep) {
		log_msg(
				"\n ========DFZ debug: in ht_search(): ep->key = %s, ep->data = %s",
				ep->key, ep->data);
		return ep;
	}
