...
#include "replication/FileWaiter-c.h"

int ffs_recvfile_c(const char *proto, const char *remote_ip,
		const char *server_port, const char *remote_filename,
		const char *local_filename);
int ffs_sendfile_c(const char *proto, const char *remote_ip,
		const char *server_port, const char *local_filename,
		const char *remote_filename);
int ffs_rmfile_c(const char *proto, const char *remote_ip,
		const char *server_port, const char *remote_filename);
...
//			ffs_recvfile_c("udt", res, "9000", fpath, fpath);//xbz: cope with replication
			c_checkoutFile("udt", res, "9000", fpath, fpath);
...
int fusion_rmdir(const char *path) {
	...
	if (ZHT_LOOKUP_FAIL != stat && !strcmp(" ", val)) {
...
int fusion_release(const char *path, struct fuse_file_info *fi) {
	...
	zht_lookup(path, nodeaddr);/*this is the address of master copy*/

	if (!strcmp(myip, nodeaddr)) {

		/*update master copy and both of the first and second replica*/
		retstat = c_checkinFile("udt", nodeaddr, "9000", fpath, fpath); //xbz:
	...
	if (iswritten) { /*so it's a write mode*/
		...
		/*update master copy and both of the first and second replica*/
		c_checkinFile2("udt", oldip, myip, "9000", fpath, fpath); //xbz:
//		zht_update(path, myip);//xbz
...
int fusion_getxattr(const char *path, const char *name, char *value,
		size_t size) {
	int retstat = 0;
	char fpath[PATH_MAX] = { 0 };
...
struct fuse_operations fusion_oper = { .getattr = fusion_getattr, .readlink =
		fusion_readlink,
		// no .getdir -- that's deprecated
		.getdir = NULL, .mknod = fusion_mknod, .mkdir = fusion_mkdir, .unlink =
				fusion_unlink, .rmdir = fusion_rmdir, .symlink = fusion_symlink,
		.rename = fusion_rename, .link = fusion_link, .chmod = fusion_chmod,
		.chown = fusion_chown, .truncate = fusion_truncate, .utime =
				fusion_utime, .open = fusion_open, .read = fusion_read, .write =
				fusion_write,
		/** Just a placeholder, don't set */ // huh???
		.statfs = fusion_statfs, .flush = fusion_flush, .release =
				fusion_release, .fsync = fusion_fsync, .setxattr =
				fusion_setxattr, .getxattr = fusion_getxattr, .listxattr =
				fusion_listxattr, .removexattr = fusion_removexattr, .opendir =
				fusion_opendir, .readdir = fusion_readdir, .releasedir =
				fusion_releasedir, .fsyncdir = fusion_fsyncdir, .init =
				fusion_init, .destroy = fusion_destroy, .access = fusion_access,
		.create = fusion_create, .ftruncate = fusion_ftruncate, .fgetattr =
				fusion_fgetattr };
